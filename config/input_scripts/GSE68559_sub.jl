...
samples = CSV.read.(filter(x->occursin(".txt",x),readdir("$cwd/data/GSE68559/GSE68559_RAW",join=true)),Ref(DataFrame),stringtype=String)
sample_names = replace.(filter(x->occursin(".txt",x),readdir("$cwd/data/GSE68559/GSE68559_RAW")),"_isoforms_expr.txt"=>"").*" data"
transcript_names = String.(strip.(select(samples[1],1)))
gene_names = String.(strip.(select(samples[1],4)))
gene_short_names = String.(strip.(select(samples[1],5)))

...
if !(isdir("$cwd/data/GSE68559_RAW"))
    ...
    run(`mv GSE68559_RAW $cwd/data/`)
    for file in readdir("$cwd/data/GSE68559_RAW")
        if occursin("het.vcf",file)
            run(`rm $cwd/data/GSE68559_RAW/$file`)
        else
            run(`gunzip $cwd/data/GSE68559_RAW/$file`)
        end
    end
...
samples = CSV.read.(filter(x->occursin(".txt",x),readdir("$cwd/data/GSE68559_RAW",join=true)),Ref(DataFrame))
...
##remove any unwanted samples from previous analysis
removed = ["GSM1675510_MB160_10 data"] 
raw_counts = raw_counts[:,setdiff(names(raw_counts),removed)]

file = "$cwd/output/cache/GSE68559_sub/raw_counts.jld2"

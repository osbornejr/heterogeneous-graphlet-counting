...
md"""
# Automorphism exploration
Here we are trying to exhausively check for each heterogeneous graphlet. 
"""

# ╔═╡ 74a350d8-31b5-4d03-b1eb-d6834edbe1df
md"""
We select a graphlet of interest:
"""
...
md"""
This graphlet has this adjacency matrix:
"""
...
md"""
## 4-node graphlet permutations
First, let's generate all possible permutations of 4-node graphlets.
"""
...
md"""
Each of these will correspond to their own $4\times 4$ adjacency matrix. We can thus calculate the eigenvalues of each and note which have the same eigenvalues as our chosen graphlet (we compare only on graphlets that have $(sum(edgelist_array)) edges.)
"""
...
md"""
And so we see that 12 adjacency matrices have the same eigenvalues as our graphlet, and we can thus check to see if these matrices are similar.
Recall that two matrices $A$ and $B$ are similar iff there is a nonsingular matrix $S$ such that 
```math
\begin{equation}
A = S^{-1}BS
\end{equation}
```
In this case, the candidates for $S$ are the permutation matrices $P$. 
"""
...
# ╔═╡ 6c3bdfd4-d7fa-49dd-866f-6e8eafc128d2
ea = eigvals(adj)

# ╔═╡ bc30c5a0-1aa4-4709-a23c-0b40722e228c
round.(eb,digits=5) == round.(ea,digits=5)

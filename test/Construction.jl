...
### Normalisation
norm_data=library_size_normalisation(clean_data,"median")
...
methods =["pcit","mutual_information","pearson","PID"]
SIMS_Mayank = Array{Array{Float64,2}}(undef,length(methods))

for (i,method) in enumerate(methods)
	@info "constructing network using $method method:"
	coexpression = method
 	similarity_matrix = coexpression_measure(sample_data,coexpression)
 	SIMS_Mayank[i] = similarity_matrix
## Adjacency matrix
 	threshold = 0.99
	...
	draw(SVG("Mayank_network_$X$coexpression.svg",16cm,16cm),gplot(g,nodefillc = nodefillc))
	
	#largest connected component plot
	nodefillc = [colorant"lightseagreen", colorant"orange"][(vertexlist_comp.=="coding").+1]
	draw(SVG("Mayank_network_$X$coexpression component.svg",16cm,16cm),gplot(g_comp,nodefillc = nodefillc))
	#Network Analysis

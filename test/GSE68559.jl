params = RunParameters("GSE68559","menu2","$cwd/website",25,"upper_quartile",0.025,"pidc",0.95,"empirical_dist_zero",1000,true,false,true,true)
...
if(biomart_modification = true)
 	biomart_raw_counts_file = "$cwd/output/cache/$(params.test_name)_raw_counts_biomart.jld"
 	if (isfile(biomart_raw_counts_file))
 		raw_counts = JLD.load(biomart_raw_counts_file,"raw counts")
 	else
 	 	#restart R session	
 	 	R"""
 	 	...
 	 	JLD.save(biomart_raw_counts_file,"raw counts",raw_counts)
 	end
end
